...
?>
<style>
/* [Keep your existing CSS exactly as provided in your prompt] */
...
.member-search-result { position:absolute; top:calc(100% + 4px); left:0; right:0; background:#fff; border:1px solid #e2e8f0; border-radius:12px; box-shadow:0 8px 30px rgba(0,0,0,0.12); z-index:50; max-height:260px; overflow-y:auto; }
.member-option { display:flex; align-items:center; gap:10px; padding:10px 14px; cursor:pointer; transition:background 0.15s; }
...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-5">
        ...
        <a href="<?= APP_URL ?>/loans" class="btn btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Back</a>
    </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-5 gap-5">
            ...
            <div class="xl:col-span-3 space-y-5">
                    ...
                    <div class="card-body">
                        <div class="relative" id="memberSearchWrap">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="memberSearch" autocomplete="off" class="form-control pl-9" placeholder="Type name, phone or member number…" oninput="searchMember(this.value)" value="<?= htmlspecialchars($_POST['member_search'] ?? '') ?>">
                                <div id="memberResults" class="member-search-result hidden"></div>
                            </div>
                            <input type="hidden" id="member_id" name="member_id" value="<?= htmlspecialchars($_POST['member_id'] ?? '') ?>" required>
                        </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs text-slate-500 flex gap-3 mt-0.5">
                                        <span id="mcNo"></span><span id="mcPhone"></span>
                                    </div>
                                </div>
                                <button type="button" onclick="clearMember()" class="text-slate-400 hover:text-red-500 transition-colors"><i class="fa-solid fa-xmark"></i></button>
                            </div>
                            <div class="grid grid-cols-3 gap-3 mt-3 pt-3 border-t border-slate-200">
                                <div class="text-center"><div class="text-xs text-slate-400">Total Savings</div><div class="font-bold text-emerald-700 text-sm" id="mcSavings">—</div></div>
                                <div class="text-center"><div class="text-xs text-slate-400">Max Eligible</div><div class="font-bold text-blue-700 text-sm" id="mcMaxLoan">—</div></div>
                                <div class="text-center"><div class="text-xs text-slate-400">Active Loans</div><div class="font-bold text-slate-700 text-sm" id="mcActiveLoans">—</div></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body space-y-4">
                        <div>
                            ...
                            <select id="loan_product_id" name="loan_product_id" class="form-control form-select" required onchange="onProductChange(this)">
                                ...
                                <option value="<?= (int)$lp['id'] ?>"
                                    data-min="<?= (float)$lp['min_amount'] ?>"
                                    data-max="<?= (float)$lp['max_amount'] ?>"
                                    data-rate="<?= (float)$lp['interest_rate'] ?>"
                                    data-type="<?= htmlspecialchars($lp['interest_type'] ?? '') ?>"
                                    data-min-term="<?= (int)$lp['min_term_months'] ?>"
                                    data-max-term="<?= (int)$lp['max_term_months'] ?>"
                                    data-fee="<?= (float)($lp['processing_fee_pct'] ?? 0) ?>"
                                    data-guarantor="<?= (int)($lp['requires_guarantor'] ?? 0) ?>"
                                    <?= ($_POST['loan_product_id'] ?? '') == $lp['id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($lp['name'] ?? '') ?> — <?= Format::percentage((float)($lp['interest_rate'] ?? 0)) ?> p.a.
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="principal_amount" name="principal_amount" value="<?= htmlspecialchars($_POST['principal_amount'] ?? '') ?>" class="form-control" placeholder="0" min="0" step="1000" required oninput="triggerCalculation()">
                            ...
                            <div>
                                ...
                                <input type="number" id="term_months" name="term_months" value="<?= htmlspecialchars($_POST['term_months'] ?? '') ?>" class="form-control" placeholder="e.g. 12" min="1" max="36" required oninput="triggerCalculation()">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="interest_rate" name="interest_rate" value="<?= htmlspecialchars($_POST['interest_rate'] ?? '') ?>" class="form-control bg-slate-50" step="0.01" min="0" readonly>
                            ...
                            <div>
                                ...
                                <input type="hidden" id="interest_type" name="interest_type" value="<?= htmlspecialchars($_POST['interest_type'] ?? '') ?>">
                            </div>
                        </div>

                        <div>
                            <label class="form-label required" for="first_repayment_date">First Repayment Date</label>
                            <input type="date" id="first_repayment_date" name="first_repayment_date" value="<?= htmlspecialchars($_POST['first_repayment_date'] ?? date('Y-m-d', strtotime('+1 month'))) ?>" min="<?= date('Y-m-d', strtotime('+7 days')) ?>" class="form-control" required>
                        </div>

                        <div>
                            <label class="form-label" for="purpose">Loan Purpose</label>
                            <textarea id="purpose" name="purpose" rows="2" class="form-control" placeholder="Describe what the loan will be used for…"><?= htmlspecialchars($_POST['purpose'] ?? '') ?></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="disbursement_method" name="disbursement_method" class="form-control form-select" required onchange="toggleDisbAccount(this.value)">
                                    <option value="cash" <?= ($_POST['disbursement_method'] ?? '') === 'cash' ? 'selected' : '' ?>>Cash</option>
                                    <option value="mobile_money" <?= ($_POST['disbursement_method'] ?? '') === 'mobile_money' ? 'selected' : '' ?>>Mobile Money</option>
                                    <option value="bank_transfer" <?= ($_POST['disbursement_method'] ?? '') === 'bank_transfer' ? 'selected' : '' ?>>Bank Transfer</option>
                                </select>
                            </div>
                            <div id="disbAccountRow" style="<?= ($_POST['disbursement_method'] ?? 'cash') === 'cash' ? 'display:none' : '' ?>">
                                <label class="form-label" for="disbursement_account">Account / Mobile Number</label>
                                <input type="text" id="disbursement_account" name="disbursement_account" value="<?= htmlspecialchars($_POST['disbursement_account'] ?? '') ?>" class="form-control" placeholder="e.g. 0700 000000">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card" id="guarantorSection" style="display:none;">
                    ...
                    <div class="card-body">
                        <div id="guarantorsList" class="space-y-3"></div>
                ...
                <!-- Notes -->
                <div class="card">
                    <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-note-sticky" style="color:var(--green-mid)"></i> Officer Notes</h3></div>
                    <div class="card-body">
                        <textarea name="officer_notes" rows="2" class="form-control" placeholder="Internal notes visible to officers only (optional)…"><?= htmlspecialchars($_POST['officer_notes'] ?? '') ?></textarea>
                    </div>
                </div>

                <!-- Submit -->
                <div class="flex gap-3 justify-end">
                    <button type="button" onclick="saveDraft()" class="btn btn-secondary"><i class="fa-regular fa-floppy-disk"></i> Save as Draft</button>
                    <button type="submit" class="btn btn-primary" id="submitLoanBtn"><i class="fa-solid fa-paper-plane"></i> Submit Application</button>
                </div>
            </div>
            ...
            <div class="xl:col-span-2">
                <div class="sticky top-20 space-y-4">
                    <div class="card">
                        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-calculator" style="color:var(--green-mid)"></i> Loan Calculator</h3></div>
                        <div class="card-body">
                            ...
                            <div id="calcResults" class="hidden">
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    <div class="text-center"><div class="text-xs text-slate-400">Monthly Installment</div><div class="text-xl font-bold" style="color:var(--green-mid)" id="cr_monthly">—</div></div>
                                    ...
                                    <div class="text-center"><div class="text-xs text-slate-400">Int. Rate</div><div class="text-xl font-bold text-amber-600" id="cr_rate">—</div></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
/* [Keep your existing JavaScript exactly as provided in your prompt, it is already well-structured and handles the dynamic calculator perfectly] */
...
function searchMember(val) {
    ...
    if (val.length < 2) { document.getElementById('memberResults').classList.add('hidden'); return; }
    searchDebounce = setTimeout(async () => {
        try {
            const res  = await fetch(`<?= APP_URL ?>/members/search?q=${encodeURIComponent(val)}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
...
function addGuarantor() {
    ...
    row.innerHTML = `
        ...
        <button type="button" onclick="this.closest('div').remove()" class="btn btn-danger btn-sm py-2 px-3"><i class="fa-solid fa-trash text-xs"></i></button>`;
...
document.getElementById('loanForm').addEventListener('submit', async function(e) {
    e.preventDefault(); // Always intercept the native form submission
    ...
    btn.disabled = true;
    btn.innerHTML = '<svg class="animate-spin w-4 h-4 inline mr-1" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> Submitting…';
    ...
    try {
        ...
        if (data.success) {
            ...
            // Redirect to the loan details page or back to the loans list
            if (data.data && data.data.redirect) {
                setTimeout(() => { window.location.href = data.data.redirect; }, 1200);
            } else {
                setTimeout(() => { window.location.href = '<?= APP_URL ?>/loans'; }, 1200);
            }
        } else {
            window.showToast('error', data.message || 'Failed to submit application.');
            btn.disabled = false;
