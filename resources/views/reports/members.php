...
$activePage = 'reports'; 
$breadcrumbs = ['Reports' => APP_URL.'/reports', 'Members' => null]; 
$filters = $filters ?? ['from' => date('Y-m-01'), 'to' => date('Y-m-d'), 'status' => ''];
?>
<div class="flex items-center justify-between mb-5">
  <div>
    <h1 class="text-xl font-bold text-slate-800">Members Report</h1>
    <p class="text-sm text-slate-400">Joined: <?= Format::date($filters['from'] ?? '') ?> — <?= Format::date($filters['to'] ?? '') ?></p>
  </div>
    ...
    <table class="data-table">
      <thead>
        <tr>
          ...
          <th class="hidden lg:table-cell">District</th>
          <th>Savings</th>
          <th class="hidden md:table-cell">Loans</th>
          <th>Status</th>
          <th class="hidden lg:table-cell">KYC</th>
          <th class="hidden md:table-cell">Joined</th>
        </tr>
      </thead>
      <tbody>
        <?php if (empty($data)): ?>
          <tr><td colspan="10" class="text-center py-10 text-slate-400">No members for this period</td></tr>
        <?php else: foreach ($data as $row): ?>
        <tr>
          <td><span class="font-mono text-xs"><?= htmlspecialchars($row['member_no'] ?? '') ?></span></td>
          <td>
            <div class="font-semibold text-sm"><?= htmlspecialchars(($row['first_name'] ?? '') . ' ' . ($row['last_name'] ?? '')) ?></div>
          </td>
          <td class="hidden md:table-cell text-xs capitalize"><?= htmlspecialchars($row['gender'] ?? '—') ?></td>
          ...
          <td class="font-bold text-emerald-700"><?= Format::currency((float)($row['total_savings'] ?? 0)) ?></td>
          <td class="hidden md:table-cell text-center"><?= (int)($row['total_loans'] ?? 0) ?></td>
          <td><?= Format::statusPill($row['status'] ?? '') ?></td>
          <td class="hidden lg:table-cell">
            <?= !empty($row['kyc_verified']) ? '<span class="text-emerald-500"><i class="fa-solid fa-check"></i></span>' : '<span class="text-amber-400"><i class="fa-solid fa-clock"></i></span>' ?>
          </td>
