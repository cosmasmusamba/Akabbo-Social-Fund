...
$pageTitle   = $pageTitle ?? 'Savings Groups';
$activePage  = $activePage ?? 'groups';
$breadcrumbs = $breadcrumbs ?? ['Groups' => null];
$groups      = $groups ?? [];
?>
            ...
            <div class="card hover:shadow-md transition-all">
                <div class="card-body">
                    <div class="flex items-start justify-between mb-3">
                        <div class="flex items-center gap-3">
                            <?php if (!empty($g['avatar'])): ?>
                                <img src="<?= APP_URL ?>/storage/uploads/avatars/<?= htmlspecialchars($g['avatar']) ?>" class="w-11 h-11 rounded-xl object-cover" alt="">
                            <?php else: ?>
                                <div class="w-11 h-11 rounded-xl flex items-center justify-center text-white font-bold text-base flex-shrink-0" style="background:linear-gradient(135deg,#136b55,#1a8f6f)">
                                    <?= strtoupper(substr($g['name'] ?? 'G', 0, 2)) ?>
                                </div>
                            <?php endif; ?>
                            <div>
                        ...
                        <?= Format::statusPill($g['status'] ?? 'active') ?>
                    </div>
                    
                    <?php if (!empty($g['description'])): ?>
                        <p class="text-sm text-slate-500 mb-3 leading-relaxed"><?= htmlspecialchars(Format::truncate($g['description'], 80)) ?></p>
                    <?php endif; ?>
                    ...
                    <div class="flex gap-2">
                        <a href="<?= APP_URL ?>/groups/<?= $g['id'] ?? 0 ?>" class="btn btn-secondary btn-sm flex-1 justify-center">
                            <i class="fa-solid fa-eye text-xs"></i> View
                        </a>
                        <?php if ($auth->can('groups.edit')): ?>
                            <a href="<?= APP_URL ?>/groups/<?= $g['id'] ?? 0 ?>/edit" class="btn btn-secondary btn-sm px-3" title="Edit group">
                                <i class="fa-solid fa-pen text-xs"></i>
                            </a>
                        <?php endif; ?>
                        <?php if ($auth->can('groups.delete') && ($g['member_count'] ?? 0) == 0): ?>
                            <button onclick="deleteGroup(<?= $g['id'] ?? 0 ?>,'<?= addslashes($g['name'] ?? '') ?>')" class="btn btn-danger btn-sm px-3" title="Delete group">
