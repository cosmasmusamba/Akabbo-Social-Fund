  ...
  <div class="card-body">
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
      ...
      <div class="flex items-start gap-3 p-4 rounded-xl" style="background:#eff6ff;border:1px solid #bfdbfe">
        ...
        <div>
          <div class="font-bold text-slate-800">+<?= $config['loan_multiplier_bonus'] ?? 1 ?>x Loan Multiplier</div>
...
<div class="card">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <div class="flex items-center gap-2.5">
              <div class="avatar-circle w-8 h-8 text-xs">
                <?= Format::initials($s['full_name']) ?>
              </div>
              <div class="font-semibold text-sm text-slate-800"><?= htmlspecialchars($s['full_name']) ?></div>
            </div>
          </td>
          <td><span class="font-mono text-xs bg-slate-100 px-2 py-0.5 rounded"><?= htmlspecialchars($s['member_no']) ?></span></td>
          <td class="hidden md:table-cell font-bold text-slate-800"><?= number_format($s['shares_held']) ?></td>
          <td class="hidden md:table-cell font-semibold text-emerald-700"><?= Format::currency((float)$s['total_invested']) ?></td>
          ...
          <td><?= Format::statusPill($s['status']) ?></td>
          <td>
            <a href="<?= APP_URL ?>/shares/member/<?= $s['member_id'] ?>"
  ...
  <?php if (!empty($result['data'])): ?>
  <?php $page=$result['page']??1; $lastPage=$result['last_page']??1; $qs=http_build_query(array_diff_key($_GET,['page'=>''])); $base=APP_URL.'/shares?'.($qs?$qs.'&':''); ?>
...
<!-- Pending share transactions -->
<?php
$pendingTxns = $result['data'] ?? []; // reuse will be replaced below via AJAX normally
?>
...
async function saveConfig() {
  await window.submitForm(document.getElementById('configForm'), {
    onSuccess: r => { window.showToast('success', r.message); closeConfigModal(); setTimeout(() => location.reload(), 900); }
  });
  // Override action
