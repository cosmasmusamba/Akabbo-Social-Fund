...
$pageTitle   = $pageTitle ?? 'Social Fund Fees';
$activePage  = $activePage ?? 'social-fund';
$breadcrumbs = $breadcrumbs ?? ['Social Fund Fees' => null];
...
<?php if ($activeFee && !empty($summary)): ?>
    <!-- Current month hero -->
        ...
        <div class="card-body">
            <div class="flex flex-col sm:flex-row items-start justify-between mb-4 gap-4">
                <div>
                    ...
                    <div class="text-2xl font-black"><?= htmlspecialchars($activeFee['name'] ?? 'Social Fund Fee') ?></div>
                    <div class="text-sm mt-0.5" style="color:#a7f3d0">
                        <?= Format::currency((float)($activeFee['amount'] ?? 0)) ?> per member ·
                        Due by <?= (int)($activeFee['due_day'] ?? 1) ?>
                        <?= in_array((int)($activeFee['due_day'] ?? 1), [1,21,31]) ? 'st' : (in_array((int)($activeFee['due_day'] ?? 1), [2,22]) ? 'nd' : (in_array((int)($activeFee['due_day'] ?? 1), [3,23]) ? 'rd' : 'th')) ?>
                        + <?= (int)($activeFee['grace_days'] ?? 5) ?> days grace
                    </div>
                </div>
                <div class="text-right w-full sm:w-auto">
                    ...
                    $rate = $totalDue > 0 ? min(100, round(($totalCollected / $totalDue) * 100)) : 0; 
                    ?>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                ...
                <div class="text-center py-3 rounded-xl" style="background:rgba(0,0,0,0.2)">
                    <div class="text-xl font-black" style="color:<?= $cl ?>"><?= number_format((int)$v) ?></div>
            ...
            <?php if (($summary['total_arrears'] ?? 0) > 0): ?>
                <div class="mt-3 px-3 py-2 rounded-xl text-sm" style="background:rgba(239,68,68,.2);color:#fca5a5">
                    ...
                    Total arrears: <strong><?= Format::currency((float)($summary['total_arrears'] ?? 0)) ?></strong>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <?php else: foreach ($fees as $f): ?>
                    <tr>
                        <td>
                            <div class="font-semibold text-slate-800"><?= htmlspecialchars($f['name'] ?? 'Fee') ?></div>
                            <?php if (($f['penalty_amount'] ?? 0) > 0): ?>
                                <div class="text-xs text-red-500 mt-0.5">
                                    <i class="fa-solid fa-exclamation-circle"></i>
                                    Late penalty: <?= Format::currency((float)($f['penalty_amount'] ?? 0)) ?>
                                </div>
                            <?php endif; ?>
                        </td>
                        <td class="font-bold text-emerald-700"><?= Format::currency((float)($f['amount'] ?? 0)) ?></td>
                        <td class="hidden md:table-cell text-sm capitalize"><?= htmlspecialchars($f['frequency'] ?? 'monthly') ?></td>
                        <td class="hidden md:table-cell text-xs text-slate-500">
                            Day <?= (int)($f['due_day'] ?? 1) ?> + <?= (int)($f['grace_days'] ?? 5) ?> days
                        </td>
                        <td class="hidden lg:table-cell text-xs capitalize"><?= htmlspecialchars(str_replace('_', ' ', $f['applies_to'] ?? 'all')) ?></td>
                        <td class="hidden lg:table-cell text-xs text-slate-400"><?= Format::date($f['effective_from'] ?? '') ?></td>
                        <td><?= Format::statusPill($f['status'] ?? 'active') ?></td>
                        <td>
                            <div class="flex gap-1">
                                <a href="<?= APP_URL ?>/social-fund/payments?fee=<?= $f['id'] ?? 0 ?>"
                                   class="btn btn-secondary btn-sm py-1 px-2.5" title="View payments">
                                    <i class="fa-solid fa-eye text-xs"></i>
                                </a>
                                <?php if ($auth->can('social_fund.manage')): ?>
                                    <a href="<?= APP_URL ?>/social-fund/<?= $f['id'] ?? 0 ?>/edit"
                                       class="btn btn-secondary btn-sm py-1 px-2.5" title="Edit">
                                        <i class="fa-solid fa-pen text-xs"></i>
                                    </a>
                                    <button onclick="generatePeriod(<?= $f['id'] ?? 0 ?>, '<?= addslashes($f['name'] ?? 'Fee') ?>')"
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="number" id="genYear" class="form-control"
                           value="<?= date('Y') ?>" min="2020" max="<?= (int)date('Y') + 1 ?>">
                </div>
            </div>
