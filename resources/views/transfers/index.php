...
<div class="card mb-4"><div class="card-body py-3">
  <div class="flex gap-2 flex-wrap">
    <?php foreach (['pending' => 'Pending', 'completed' => 'Completed', 'rejected' => 'Rejected', 'reversed' => 'Reversed', '' => 'All'] as $v => $l): ?>
    <a href="?status=<?= $v ?>"
       class="btn btn-sm <?= ($filters['status'] ?? '') === $v ? 'btn-primary' : 'btn-secondary' ?>">
      <?= $l ?>
    </a>
    <?php endforeach; ?>
  </div>
...
<div class="card">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="font-bold text-slate-800"><?= Format::currency((float)$t['amount']) ?></td>
  ...
  <?php if (!empty($result['data'])): $page=$result['page']??1;$lp=$result['last_page']??1;$qs=http_build_query(array_diff_key($_GET,['page'=>'']));$base=APP_URL.'/transfers?'.($qs?$qs.'&':''); ?>
  <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between">
    ...
    <div class="flex gap-1">
      ...
      <?php for($p=max(1,$page-2);$p<=min($lp,$page+2);$p++): ?><a href="<?= $base ?>page=<?= $p ?>" class="pager-btn <?= $p==$page?'active':'' ?>"><?= $p ?></a><?php endfor; ?>
      <a href="<?= $base ?>page=<?= min($lp,$page+1) ?>" class="pager-btn <?= $page>=$lp?'opacity-40 pointer-events-none':'' ?>"><i class="fa-solid fa-chevron-right text-xs"></i></a>
    </div>
  </div>
  <?php endif; ?>
</div>