...
$pageTitle   = $isEdit ? 'Edit: ' . $product['name'] : 'New Loan Product';
$activePage  = 'loan-products';
$breadcrumbs = ['Loan Products' => APP_URL . '/loan-products', $isEdit ? 'Edit' : 'New' => null];
$action      = $isEdit
    ? APP_URL . '/loan-products/' . $product['id'] . '/update'
    : APP_URL . '/loan-products/store';
?>
<div class="max-w-3xl mx-auto">
  <div class="flex items-center justify-between mb-5">
    <div>
      <h1 class="text-xl font-bold text-slate-800"><?= $isEdit ? 'Edit Loan Product' : 'New Loan Product' ?></h1>
      <p class="text-sm text-slate-400 mt-0.5">
        <?= $isEdit ? htmlspecialchars($product['name']) : 'Configure a new loan type for members' ?>
      </p>
    </div>
  ...
  <form id="productForm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="card-body space-y-4">
          <div>
            ...
            <input type="text" name="name" class="form-control"
                   ...
                   placeholder="e.g. Emergency Loan, Business Loan" required>
          </div>
          <div>
            <label class="form-label">Description</label>
            <textarea name="description" rows="3" class="form-control"
                      placeholder="Brief description shown to officers when creating loans…"><?= htmlspecialchars($product['description'] ?? '') ?></textarea>
          </div>
          <?php if ($isEdit): ?>
          <div>
        ...
        <div class="card-body space-y-4">
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select name="interest_type" class="form-control form-select" required onchange="updatePreview()">
                <?php foreach (['flat' => 'Flat Rate', 'reducing_balance' => 'Reducing Balance', 'compound' => 'Compound'] as $val => $label): ?>
                <option value="<?= $val ?>" <?= ($product['interest_type'] ?? 'reducing_balance') === $val ? 'selected' : '' ?>>
                  <?= $label ?>
                </option>
                <?php endforeach; ?>
              </select>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" name="min_term_months" class="form-control"
                     value="<?= $product['min_term_months'] ?? 1 ?>"
                     min="1" max="60" required>
            </div>
            <div>
              <label class="form-label required">Max Term (months)</label>
              <input type="number" name="max_term_months" class="form-control"
                     value="<?= $product['max_term_months'] ?? 12 ?>"
                     min="1" max="60" required>
            </div>
          </div>
          <div>
            ...
            <input type="number" name="max_loan_multiplier" class="form-control"
                   value="<?= $product['max_loan_multiplier'] ?? 3 ?>"
                   min="1" max="10" step="1">
      ...
      <div class="card">
        ...
        <div class="card-body space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="form-label required">Minimum Amount (<?= $settings['currency_symbol'] ?? 'USh' ?>)</label>
              <input type="number" name="min_amount" class="form-control"
                     value="<?= $product['min_amount'] ?? '' ?>"
                     min="0" step="1000" placeholder="50,000" required
                     oninput="updatePreview()">
            </div>
            <div>
              <label class="form-label required">Maximum Amount (<?= $settings['currency_symbol'] ?? 'USh' ?>)</label>
              <input type="number" name="max_amount" class="form-control"
                     value="<?= $product['max_amount'] ?? '' ?>"
                     min="0" step="1000" placeholder="5,000,000" required>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" name="processing_fee_pct" class="form-control"
                     value="<?= $product['processing_fee_pct'] ?? 0 ?>"
                     step="0.01" min="0" max="20" placeholder="1.00">
            </div>
            <div>
              <label class="form-label">Insurance Fee (%)</label>
              <input type="number" name="insurance_fee_pct" class="form-control"
                     value="<?= $product['insurance_fee_pct'] ?? 0 ?>"
          ...
          <div class="space-y-3 pt-2">
            <label class="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" name="requires_guarantor" value="1" class="w-4 h-4 rounded"
                     style="accent-color:var(--green-mid)"
                     <?= ($product['requires_guarantor'] ?? 0) ? 'checked' : '' ?>>
              <div>
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" name="requires_collateral" value="1" class="w-4 h-4 rounded"
                     style="accent-color:var(--green-mid)"
                     <?= ($product['requires_collateral'] ?? 0) ? 'checked' : '' ?>>
              <div>
    ...
    <div class="flex gap-3 justify-end mt-5">
      ...
      <button type="submit" class="btn btn-primary">
        <i class="fa-solid fa-save"></i> <?= $isEdit ? 'Save Changes' : 'Create Product' ?>
      </button>
    </div>
  </form>
</div>
...
function updatePreview() {
  ...
  document.getElementById('previewContent').innerHTML = `
    ...
    <div class="text-center"><div class="text-lg font-black text-slate-800">${fmt(totalPayable)}</div><div class="text-xs text-slate-400 mt-0.5">Total Payable</div></div>
  `;
}
...
document.getElementById('productForm').addEventListener('submit', async function(e) {
  ...
  await window.submitForm(this, {
    ...
    onSuccess: r => {
      ...
      setTimeout(() => { window.location.href = r.data?.redirect || '<?= APP_URL ?>/loan-products'; }, 800);
    }
  });
});

// Override the form action dynamically
document.getElementById('productForm').action = FORM_ACTION;
</script>
