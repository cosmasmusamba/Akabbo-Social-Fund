...
$pageTitle   = 'Edit: ' . ($product['name'] ?? 'Loan Product');
...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-5">
        <div>
            <h1 class="text-xl font-bold text-slate-800">Edit Loan Product</h1>
            <p class="text-sm text-slate-400 mt-0.5"><?= htmlspecialchars($product['name'] ?? '') ?></p>
        </div>
            ...
            <div class="card">
                ...
                <div class="card-body space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" class="form-control" value="<?= htmlspecialchars($product['name'] ?? '') ?>" placeholder="e.g. Emergency Loan, Business Loan" required>
                    </div>
                    <div>
                        <label class="form-label">Description</label>
                        <textarea name="description" rows="3" class="form-control" placeholder="Brief description shown to officers when creating loans…"><?= htmlspecialchars($product['description'] ?? '') ?></textarea>
                    </div>
                    <div>
                ...
                <div class="card-body space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="interest_rate" class="form-control" value="<?= htmlspecialchars($product['interest_rate'] ?? '') ?>" step="0.01" min="0" max="100" placeholder="10.00" required oninput="updatePreview()">
                        </div>
                        <div>
                            ...
                            <select name="interest_type" class="form-control form-select" required onchange="updatePreview()">
                                <option value="flat" <?= ($product['interest_type'] ?? '') === 'flat' ? 'selected' : '' ?>>Flat Rate</option>
                                <option value="reducing_balance" <?= ($product['interest_type'] ?? 'reducing_balance') === 'reducing_balance' ? 'selected' : '' ?>>Reducing Balance</option>
                                <option value="compound" <?= ($product['interest_type'] ?? '') === 'compound' ? 'selected' : '' ?>>Compound</option>
                            </select>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="min_term_months" class="form-control" value="<?= (int)($product['min_term_months'] ?? 1) ?>" min="1" max="60" required>
                        </div>
                        <div>
                            <label class="form-label required">Max Term (months)</label>
                            <input type="number" name="max_term_months" class="form-control" value="<?= (int)($product['max_term_months'] ?? 12) ?>" min="1" max="60" required>
                        </div>
                    </div>
                    <div>
                        <label class="form-label">Max Loan Multiplier</label>
                        <input type="number" name="max_loan_multiplier" class="form-control" value="<?= (int)($product['max_loan_multiplier'] ?? 3) ?>" min="1" max="10" step="1">
                ...
                <div class="card-body space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="min_amount" class="form-control" value="<?= htmlspecialchars($product['min_amount'] ?? '') ?>" min="0" step="1000" placeholder="50,000" required oninput="updatePreview()">
                        </div>
                        <div>
                            <label class="form-label required">Maximum Amount (<?= htmlspecialchars($currency) ?>)</label>
                            <input type="number" name="max_amount" class="form-control" value="<?= htmlspecialchars($product['max_amount'] ?? '') ?>" min="0" step="1000" placeholder="5,000,000" required>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="processing_fee_pct" class="form-control" value="<?= htmlspecialchars($product['processing_fee_pct'] ?? 0) ?>" step="0.01" min="0" max="20" placeholder="1.00">
                        </div>
                        <div>
                            <label class="form-label">Insurance Fee (%)</label>
                            <input type="number" name="insurance_fee_pct" class="form-control" value="<?= htmlspecialchars($product['insurance_fee_pct'] ?? 0) ?>" step="0.01" min="0" max="10" placeholder="0.00">
                        </div>
                    </div>
                    <div class="space-y-3 pt-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="requires_guarantor" value="1" class="w-4 h-4 rounded" style="accent-color:var(--green-mid)" <?= !empty($product['requires_guarantor']) ? 'checked' : '' ?>>
                            <div>
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="requires_collateral" value="1" class="w-4 h-4 rounded" style="accent-color:var(--green-mid)" <?= !empty($product['requires_collateral']) ? 'checked' : '' ?>>
                            <div>
...
const FORM_ACTION = '<?= APP_URL ?>/loan-products/' + <?= (int)($product['id'] ?? 0) ?> + '/update';
...
// Trigger preview on load if data exists
