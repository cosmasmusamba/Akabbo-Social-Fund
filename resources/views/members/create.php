...
$activePage  = 'member-create';
$breadcrumbs = ['Members' => APP_URL.'/members', 'Register Member' => null];
?>
...
<style>
  ...
  .step-num {
    ...
    font-size:0.8rem; font-weight:700; border:2px solid; flex-shrink:0; transition:all 0.3s;
  }
  ...
  /* Photo upload zone */
  .photo-zone {
    width:110px; height:110px; border-radius:50%; border:2px dashed #d1d5db;
    display:flex; flex-direction:column; align-items:center; justify-content:center;
    cursor:pointer; transition:all 0.2s; overflow:hidden; background:#f9fafb; position:relative;
  }
  .photo-zone:hover { border-color:var(--green-mid); background:#f0fdf9; }
  ...
  .form-section-title {
    ...
    border-bottom:2px solid #ecfdf5; display:flex; align-items:center; gap:8px;
  }
  ...
  .summary-card h4 { font-size:0.8rem; font-weight:700; color:var(--green-mid); margin-bottom:12px; border-bottom:1px solid #e2e8f0; padding-bottom:6px; }
...
<div class="max-w-4xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-5">
    <div>
      <h1 class="text-xl font-bold text-slate-800">Register New Member</h1>
      <p class="text-sm text-slate-400 mt-0.5">Complete all required fields to enroll a member</p>
    </div>
  ...
  <!-- Form -->
  <form id="memberForm" method="POST" action="<?= APP_URL ?>/members/store" enctype="multipart/form-data" novalidate>
    <?= $csrfField ?>

    <!-- STEP 1: Personal Info -->
    <div class="form-panel active" id="panel-1">
      <div class="card">
        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-user" style="color:var(--green-mid)"></i> Personal Information</h3></div>
        <div class="card-body space-y-5">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5 pb-4 border-b border-slate-100">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-slate-400">Upload a clear passport-size photo. JPG or PNG, max 2MB. A good photo helps with identity verification.</p>
              <button type="button" onclick="document.getElementById('avatarInput').click()" class="mt-2 text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"><i class="fa-solid fa-upload mr-1"></i> Choose Photo</button>
            </div>
          </div>
          <!-- Name fields -->
          <div><div class="form-section-title"><i class="fa-solid fa-id-card"></i> Full Name</div><div class="grid-3">
            <div><label class="form-label required" for="first_name">First Name</label><input type="text" id="first_name" name="first_name" value="<?= htmlspecialchars($_POST['first_name'] ?? '') ?>" class="form-control" placeholder="e.g. Amara" required></div>
            <div><label class="form-label" for="middle_name">Middle Name</label><input type="text" id="middle_name" name="middle_name" value="<?= htmlspecialchars($_POST['middle_name'] ?? '') ?>" class="form-control" placeholder="Optional"></div>
            <div><label class="form-label required" for="last_name">Last Name / Surname</label><input type="text" id="last_name" name="last_name" value="<?= htmlspecialchars($_POST['last_name'] ?? '') ?>" class="form-control" placeholder="e.g. Nakato" required></div>
          </div></div>
          <!-- Demographics -->
          <div><div class="form-section-title"><i class="fa-solid fa-circle-info"></i> Demographics</div><div class="grid-3">
            <div><label class="form-label required" for="gender">Gender</label><select id="gender" name="gender" class="form-control form-select" required><option value="">Select gender</option><option value="male" <?= ($_POST['gender']??'')==='male'?'selected':'' ?>>Male</option><option value="female" <?= ($_POST['gender']??'')==='female'?'selected':'' ?>>Female</option><option value="other" <?= ($_POST['gender']??'')==='other'?'selected':'' ?>>Other</option></select></div>
            <div><label class="form-label" for="date_of_birth">Date of Birth</label><input type="date" id="date_of_birth" name="date_of_birth" value="<?= htmlspecialchars($_POST['date_of_birth'] ?? '') ?>" max="<?= date('Y-m-d', strtotime('-16 years')) ?>" class="form-control"><p class="field-hint">Must be at least 16 years old</p></div>
            <div><label class="form-label" for="occupation">Occupation</label><input type="text" id="occupation" name="occupation" value="<?= htmlspecialchars($_POST['occupation'] ?? '') ?>" class="form-control" placeholder="e.g. Teacher, Farmer"></div>
            <div><label class="form-label" for="employer">Employer / Business</label><input type="text" id="employer" name="employer" value="<?= htmlspecialchars($_POST['employer'] ?? '') ?>" class="form-control" placeholder="Employer or business name"></div>
            <div><label class="form-label" for="district">District</label><input type="text" id="district" name="district" value="<?= htmlspecialchars($_POST['district'] ?? '') ?>" class="form-control" placeholder="e.g. Kampala, Wakiso"></div>
          </div></div>
          <!-- Address -->
          <div><label class="form-label" for="address">Physical Address</label><textarea id="address" name="address" rows="2" class="form-control" placeholder="Village, Parish, Sub-county, District…"><?= htmlspecialchars($_POST['address'] ?? '') ?></textarea></div>
        </div>
    ...
    <div class="form-panel" id="panel-2">
      <div class="card">
        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-phone" style="color:var(--green-mid)"></i> Contact & Identity Verification</h3></div>
        <div class="card-body space-y-5">
          <!-- Contact -->
          <div><div class="form-section-title"><i class="fa-solid fa-address-book"></i> Contact Details</div><div class="grid-3">
            <div><label class="form-label required" for="phone">Primary Phone</label><input type="tel" id="phone" name="phone" value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>" class="form-control" placeholder="+256 700 000000" required><p class="field-hint">Include country code</p></div>
            <div><label class="form-label" for="phone_alt">Alternative Phone</label><input type="tel" id="phone_alt" name="phone_alt" value="<?= htmlspecialchars($_POST['phone_alt'] ?? '') ?>" class="form-control" placeholder="+256 700 000000"></div>
            <div><label class="form-label" for="email">Email Address</label><input type="email" id="email" name="email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" class="form-control" placeholder="email@example.com"><p class="field-hint">For notifications (optional)</p></div>
          </div></div>
          <!-- KYC Documents -->
          <div><div class="form-section-title"><i class="fa-solid fa-id-badge"></i> Identity Documents (KYC)</div>
            <div class="grid-2">
              <div><label class="form-label required" for="national_id">National ID Number</label><input type="text" id="national_id" name="national_id" value="<?= htmlspecialchars($_POST['national_id'] ?? '') ?>" class="form-control" placeholder="CM90000000XYZW" maxlength="20" required><p class="field-hint">Ugandan National ID (NIN)</p></div>
              <div><label class="form-label" for="passport_no">Passport Number</label><input type="text" id="passport_no" name="passport_no" value="<?= htmlspecialchars($_POST['passport_no'] ?? '') ?>" class="form-control" placeholder="A00000000"><p class="field-hint">If applicable</p></div>
            </div>
            <div class="mt-4 grid-2">
              <!-- ID Front -->
              <div>
                <label class="form-label">National ID Front</label>
                <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:border-green-400 hover:bg-green-50 transition-all cursor-pointer" onclick="document.getElementById('id_front').click()">
                  <i class="fa-regular fa-image text-2xl text-slate-300 mb-1 block"></i>
                  <p class="text-xs text-slate-400">Click to upload ID front</p>
                  <p class="text-[10px] text-slate-300 mt-0.5">JPG, PNG or PDF · Max 2MB</p>
                  <input type="file" id="id_front" name="id_front" class="hidden" accept="image/jpeg,image/png,image/webp,application/pdf">
                </div>
                <div id="id_front_preview" class="mt-2 hidden">
                  <div class="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                    ...
                    <button type="button" onclick="clearFile('id_front')" class="text-red-500 text-xs"><i class="fa-regular fa-trash-can"></i> Remove</button>
                  </div>
                </div>
              </div>
              <!-- ID Back -->
              <div>
                ...
                <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:border-green-400 hover:bg-green-50 transition-all cursor-pointer" onclick="document.getElementById('id_back').click()">
                  <i class="fa-regular fa-image text-2xl text-slate-300 mb-1 block"></i>
                  <p class="text-xs text-slate-400">Click to upload ID back</p>
                  <p class="text-[10px] text-slate-300 mt-0.5">JPG, PNG or PDF · Max 2MB</p>
                  <input type="file" id="id_back" name="id_back" class="hidden" accept="image/jpeg,image/png,image/webp,application/pdf">
                </div>
                <div id="id_back_preview" class="mt-2 hidden">
                  <div class="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                    ...
                    <button type="button" onclick="clearFile('id_back')" class="text-red-500 text-xs"><i class="fa-regular fa-trash-can"></i> Remove</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="form-panel" id="panel-3">
      <div class="card">
        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-people-group" style="color:var(--green-mid)"></i> Next of Kin / Emergency Contact</h3></div>
        <div class="card-body">
          <div class="p-4 rounded-xl mb-5" style="background:#fffbeb;border:1px solid #fde68a;"><p class="text-sm text-amber-700"><i class="fa-solid fa-triangle-exclamation mr-1.5"></i> Next of kin information is used in emergency situations and for notification purposes. Please ensure accuracy.</p></div>
          <div class="grid-3">
            <div><label class="form-label required" for="next_of_kin_name">Full Name</label><input type="text" id="next_of_kin_name" name="next_of_kin_name" value="<?= htmlspecialchars($_POST['next_of_kin_name'] ?? '') ?>" class="form-control" placeholder="Next of kin's full name" required></div>
            <div><label class="form-label required" for="next_of_kin_phone">Phone Number</label><input type="tel" id="next_of_kin_phone" name="next_of_kin_phone" value="<?= htmlspecialchars($_POST['next_of_kin_phone'] ?? '') ?>" class="form-control" placeholder="+256 700 000000" required></div>
            <div><label class="form-label required" for="next_of_kin_relationship">Relationship</label><select id="next_of_kin_relationship" name="next_of_kin_relationship" class="form-control form-select" required><option value="">Select relationship</option><?php foreach(['Spouse','Parent','Child','Sibling','Guardian','Relative','Friend','Colleague','Other'] as $r): ?><option value="<?= $r ?>" <?= ($_POST['next_of_kin_relationship']??'')===$r?'selected':'' ?>><?= $r ?></option><?php endforeach; ?></select></div>
          </div>
        </div>
        <div class="px-6 pb-5 flex justify-between">
    ...
    <div class="form-panel" id="panel-4">
      <div class="card">
        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-star" style="color:var(--green-mid)"></i> Membership Details</h3></div>
        <div class="card-body space-y-5">
          <div class="grid-2">
            <div><label class="form-label required" for="membership_date">Membership Date</label><input type="date" id="membership_date" name="membership_date" value="<?= htmlspecialchars($_POST['membership_date'] ?? date('Y-m-d')) ?>" max="<?= date('Y-m-d') ?>" class="form-control" required></div>
            <div><label class="form-label" for="group_id">Assign to Group <span class="text-slate-400 font-normal">(optional)</span></label><select id="group_id" name="group_id" class="form-control form-select"><option value="">— No group —</option><?php foreach ($savings_groups ?? [] as $g): ?><option value="<?= $g['id'] ?>" <?= ($_POST['group_id']??'')==$g['id']?'selected':'' ?>><?= htmlspecialchars($g['name']) ?></option><?php endforeach; ?></select></div>
          </div>
          <div class="p-5 rounded-xl" style="background:#f0fdf9;border:1px solid #a7f3d0;">
            <div class="flex items-center justify-between mb-3"><div><p class="font-semibold text-emerald-800 text-sm">Membership Registration Fee</p><p class="text-xs text-emerald-600">Default: <?= Format::currency((float)($settings['membership_fee'] ?? 10000)) ?></p></div><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="membership_fee_paid" id="feePaid" value="1" <?= !empty($_POST['membership_fee_paid']) ? 'checked' : '' ?> class="w-5 h-5" style="accent-color:var(--green-mid)"><span class="text-sm font-semibold text-emerald-800">Fee Paid</span></label></div>
            <div id="feeAmountRow" class="<?= empty($_POST['membership_fee_paid']) ? 'hidden' : '' ?>"><label class="form-label" for="membership_fee_amount">Amount Paid</label><div class="relative"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-slate-500"><?= htmlspecialchars($settings['currency_symbol'] ?? 'USh') ?></span><input type="number" id="membership_fee_amount" name="membership_fee_amount" value="<?= htmlspecialchars($_POST['membership_fee_amount'] ?? $settings['membership_fee'] ?? 10000) ?>" class="form-control pl-12" min="0" step="500"></div></div>
          </div>
        </div>
        <div class="px-6 pb-5 flex justify-between">
    ...
    <div class="form-panel" id="panel-5">
      <div class="card">
        <div class="card-header"><h3 class="section-title"><i class="fa-solid fa-clipboard-list" style="color:var(--green-mid)"></i> Review & Submit</h3></div>
        <div class="card-body">
        ...
        <div class="px-6 pb-5 flex justify-between items-center">
          ...
          <button type="submit" class="btn btn-primary" id="submitBtn"><i class="fa-solid fa-user-check"></i> Register Member</button>
        </div>
      </div>
    </div>
...
function updateReviewSummary() {
  ...
  document.getElementById('rev_id_front').textContent = idFrontInput && idFrontInput.files && idFrontInput.files[0] ? idFrontInput.files[0].name : 'Not uploaded';
  document.getElementById('rev_id_back').textContent = idBackInput && idBackInput.files && idBackInput.files[0] ? idBackInput.files[0].name : 'Not uploaded';
  ...
  // 🔥 FIXED: Use JavaScript number formatting, not PHP Format::currency
  ...
  document.getElementById('rev_avatar').textContent = avatarInput && avatarInput.files && avatarInput.files[0] ? avatarInput.files[0].name : 'Not uploaded';
}
...
document.getElementById('avatarInput').addEventListener('change', function() {
  ...
  reader.onload = e => { document.getElementById('photoPreview').src = e.target.result; document.getElementById('photoPreview').classList.remove('hidden'); document.getElementById('photoPlaceholder').classList.add('hidden'); };
  reader.readAsDataURL(file);
});

// Membership fee toggle
document.getElementById('feePaid').addEventListener('change', function() { document.getElementById('feeAmountRow').classList.toggle('hidden', !this.checked); });
...
setupDocPreview('id_front', 'id_front_preview', 'id_front_thumb', 'id_front_icon', 'id_front_filename');
setupDocPreview('id_back', 'id_back_preview', 'id_back_thumb', 'id_back_icon', 'id_back_filename');
...
document.getElementById('memberForm').addEventListener('submit', async function(e) {
  ...
  btn.disabled = true;
  btn.innerHTML = '<svg class="animate-spin w-4 h-4 inline mr-1" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> Registering…';
  try {
    ...
    if (data.success) {
      ...
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-user-check"></i> Register Member';
    }
  } catch (err) {
