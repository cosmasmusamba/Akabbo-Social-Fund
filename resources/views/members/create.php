...
$pageTitle   = $pageTitle ?? 'Register New Member';
$activePage  = $activePage ?? 'members';
$breadcrumbs = $breadcrumbs ?? ['Members' => APP_URL.'/members', 'Register Member' => null];
$savings_groups = $savings_groups ?? [];
?>
<style>
/* Step indicator and form styles (kept identical to your original for consistency) */
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-5">
        ...
        <a href="<?= APP_URL ?>/members" class="btn btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Back</a>
    </div>
    ...
    <form id="memberForm" method="POST" action="<?= APP_URL ?>/members/store" enctype="multipart/form-data" novalidate>
        ...
        <div class="form-panel" id="panel-3">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="p-4 rounded-xl mb-5" style="background:#fffbeb;border:1px solid #fde68a;">
                        <p class="text-sm text-amber-700"><i class="fa-solid fa-triangle-exclamation mr-1.5"></i> Next of kin information is used in emergency situations. Please ensure accuracy.</p>
                    </div>
                    <div class="grid-3">
                ...
                <div class="card-body space-y-5">
                    ...
                    <div class="p-5 rounded-xl" style="background:#f0fdf9;border:1px solid #a7f3d0;">
                        <div class="flex items-center justify-between mb-3">
                            <div><p class="font-semibold text-emerald-800 text-sm">Membership Registration Fee</p><p class="text-xs text-emerald-600">Default: <?= Format::currency((float)($settings['membership_fee'] ?? 10000)) ?></p></div>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="membership_fee_paid" id="feePaid" value="1" <?= !empty($_POST['membership_fee_paid']) ? 'checked' : '' ?> class="w-5 h-5" style="accent-color:var(--green-mid, #136b55)"><span class="text-sm font-semibold text-emerald-800">Fee Paid</span></label>
                        </div>
                        <div id="feeAmountRow" class="<?= empty($_POST['membership_fee_paid']) ? 'hidden' : '' ?>">
                            <label class="form-label" for="membership_fee_amount">Amount Paid</label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-slate-500"><?= htmlspecialchars($settings['currency_symbol'] ?? 'USh') ?></span>
                                <input type="number" id="membership_fee_amount" name="membership_fee_amount" value="<?= htmlspecialchars($_POST['membership_fee_amount'] ?? ($settings['membership_fee'] ?? 10000)) ?>" class="form-control pl-12" min="0" step="500">
                            </div>
                        </div>
                    </div>
                </div>
...
function validateStep(step) {
    const required = {1: ['first_name','last_name','gender'], 2: ['phone','national_id'], 3: ['next_of_kin_name','next_of_kin_phone','next_of_kin_relationship'], 4: ['membership_date']};
    let valid = true;
...
document.getElementById('memberForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.data && data.data.redirect) { setTimeout(() => { window.location.href = data.data.redirect; }, 800); } 
