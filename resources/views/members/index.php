<?php
// also guard http://localhost/akabbo/members/1/edit the same way http://localhost/akabbo/members/1 was guarded with  (RBAC + IDOR) and make my prifile active accordingly.
use App\Helpers\Format;
use App\Helpers\Avatar;
$pageTitle   = $pageTitle ?? 'Members';
$activePage  = $activePage ?? 'members';
$breadcrumbs = $breadcrumbs ?? ['Members' => null];
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
    <div>
        ...
        <p class="text-sm text-slate-400 mt-0.5"><?= number_format($result['total'] ?? 0) ?> total members registered</p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
        <a href="<?= APP_URL ?>/import" class="btn btn-secondary btn-sm">
        ...
        <form method="GET" class="flex flex-col sm:flex-row gap-3 flex-wrap" id="filterForm">
            <div class="relative flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>"
                       placeholder="Search name, phone, member no…" class="form-control pl-9 py-2 text-sm">
            </div>
            <select name="status" class="form-control form-select py-2 text-sm w-full sm:w-auto min-w-[140px]" onchange="this.closest('form').submit()">
                <option value="">All Statuses</option>
                <?php foreach (['active', 'inactive', 'suspended', 'exited'] as $s): ?>
                    <option value="<?= $s ?>" <?= ($_GET['status'] ?? '') === $s ? 'selected' : '' ?>><?= ucfirst($s) ?></option>
                <?php endforeach; ?>
            </select>
            <select name="kyc" class="form-control form-select py-2 text-sm w-full sm:w-auto min-w-[140px]" onchange="this.closest('form').submit()">
            ...
            <?php if (!empty($_GET['search']) || !empty($_GET['status']) || isset($_GET['kyc'])): ?>
                <a href="<?= APP_URL ?>/members" class="btn btn-secondary btn-sm"><i class="fa-solid fa-xmark"></i> Clear</a>
            <?php endif; ?>
        </form>
    </div>
</div>
        ...
        <table class="data-table">
            ...
            <?php else: ?>
                <?php foreach ($result['data'] as $m): ?>
                    <tr class="member-row" data-id="<?= $m['id'] ?? 0 ?>">
                        <td><input type="checkbox" class="row-check" value="<?= $m['id'] ?? 0 ?>"></td>
                        <td>
                        ...
                        <td>
                            <!-- // Lets have disable, activate options of members. -->
                            <div class="flex items-center gap-1">
                                ...
                                <div class="relative" data-dropdown>
                                    ...
                                    <div class="row-menu hidden absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-lg border border-slate-200 py-1 z-10">
                                        ...
                                        <?php if ($auth->can('members.edit') && empty($m['kyc_verified'])): ?>
                                            <div class="border-t border-slate-100 my-1"></div>
                                            <button onclick="sendKycReminderFromList(<?= $m['id'] ?>,'<?= addslashes(($m['first_name'] ?? '') . ' ' . ($m['last_name'] ?? '')) ?>')"
                                                    class="flex items-center gap-2.5 px-3.5 py-2 text-sm text-amber-600 hover:bg-amber-50 w-full text-left">
                                                <i class="fa-solid fa-bell w-4"></i> Send KYC Reminder
                                            </button>
                                        <?php endif; ?>
                                        ...
                                        <?php if ($auth->can('members.delete')): ?>
                                            <div class="border-t border-slate-100 mt-1 pt-1">
                                                <button onclick="confirmDelete(<?= $m['id'] ?>,'<?= addslashes(($m['first_name'] ?? '') . ' ' . ($m['last_name'] ?? '')) ?>')"
                                                        class="flex items-center gap-2.5 px-3.5 py-2 text-sm text-red-600 hover:bg-red-50 w-full text-left">
    ...
    ?>
        <div class="px-5 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-1.5">
                ...
                if (!in_array(1, $range)) {
                    echo "<a href='{$base}page=1' class='pager-btn'>1</a>";
                    if (min($range) > 2) echo "<span class='text-slate-400 text-xs px-1'>…</span>";
                }
                foreach ($range as $p): ?>
                    <a href="<?= $base ?>page=<?= $p ?>" class="pager-btn <?= $p == $page ? 'active' : '' ?>"><?= $p ?></a>
                <?php endforeach;
                if (!in_array($lastPage, $range) && $lastPage > 1) {
                    if (max($range) < $lastPage - 1) echo "<span class='text-slate-400 text-xs px-1'>…</span>";
                    echo "<a href='{$base}page={$lastPage}' class='pager-btn'>{$lastPage}</a>";
                }
                ?>
...
async function sendKycReminderFromList(id, name) {
    window.confirmAction({
        title: 'Send KYC Reminder',
        message: `Send a KYC verification reminder notification to ${name}?`,
        confirmText: 'Send',
        type: 'primary',
        onConfirm: async () => {
            try {
                ...
                const r = await fetch(`<?= APP_URL ?>/members/${id}/send-kyc-reminder`, {
                    method: 'POST',
                ...
                window.showToast('error', 'Failed to send reminder.');
            }
        }
    });
}

// Intercept the Delete Form submission via AJAX
document.getElementById('deleteForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            closeDeleteModal();
            setTimeout(() => {
                window.location.href = data.data?.redirect || '<?= APP_URL ?>/members';
            }, 1000);
        } else {
            window.showToast('error', data.message || 'Deletion failed.');
            btn.disabled = false;
