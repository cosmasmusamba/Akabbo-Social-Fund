...
$breadcrumbs = [
    ...
    $member['first_name'] . ' ' . $member['last_name'] => APP_URL . '/members/' . $member['id'],
    'Edit' => null
];
?>

<!-- Same CSS as create.php – include it unchanged -->
<style>
  /* Paste the exact same CSS block from create.php here (including summary styles) */
  ...
  .form-section-title { font-size:0.8rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; color:var(--green-mid); margin-bottom:14px; padding-bottom:6px; border-bottom:2px solid #ecfdf5; display:flex; align-items:center; gap:8px; }
...
<div class="max-w-4xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-5">
    <div><h1 class="text-xl font-bold text-slate-800">Edit Member</h1><p class="text-sm text-slate-400 mt-0.5"><?= htmlspecialchars($member['member_no']) ?> · <?= Format::statusPill($member['status']) ?></p></div>
    <a href="<?= APP_URL ?>/members/<?= $member['id'] ?>" class="btn btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Back</a>
  </div>

  <!-- Step indicator (5 steps) -->
  <div class="card mb-5 px-6 py-4"><div class="step-indicator"><div class="step active" id="step-ind-1"><div class="step-num">1</div><div class="step-label hidden sm:block">Personal Info</div></div><div class="step-line" id="line-1"></div><div class="step pending" id="step-ind-2"><div class="step-num">2</div><div class="step-label hidden sm:block">Contact & KYC</div></div><div class="step-line" id="line-2"></div><div class="step pending" id="step-ind-3"><div class="step-num">3</div><div class="step-label hidden sm:block">Next of Kin</div></div><div class="step-line" id="line-3"></div><div class="step pending" id="step-ind-4"><div class="step-num">4</div><div class="step-label hidden sm:block">Membership</div></div><div class="step-line" id="line-4"></div><div class="step pending" id="step-ind-5"><div class="step-num">5</div><div class="step-label hidden sm:block">Review & Submit</div></div></div></div>

  <form id="memberForm" method="POST" action="<?= APP_URL ?>/members/<?= $member['id'] ?>/update" enctype="multipart/form-data" novalidate>
    <?= $csrfField ?>
      ...
      <div class="card">
        ...
        <div class="card-body space-y-5">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5 pb-4 border-b border-slate-100">
            <div><div class="photo-zone" id="photoZone" onclick="document.getElementById('avatarInput').click()">
            ...
            </div><input type="file" id="avatarInput" name="avatar" accept="image/jpeg,image/png,image/webp" class="hidden"></div>
            <div class="flex-1"><p class="text-sm font-medium text-slate-700 mb-1">Member Photo <span class="text-slate-400 font-normal">(optional)</span></p><p class="text-xs text-slate-400">Upload a clear passport-size photo. JPG or PNG, max 2MB.</p><button type="button" onclick="document.getElementById('avatarInput').click()" class="mt-2 text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"><i class="fa-solid fa-upload mr-1"></i> Change Photo</button></div>
          </div>
          <!-- Name fields -->
                  ...
                  <div>
                      ...
                      <?php if (!empty($member['id_front'])): ?>
                          <div class="mb-2 flex items-center gap-2 bg-slate-50 rounded-lg p-2" id="existing_front_block">
                              <?php 
                              $frontExt = pathinfo($member['id_front'], PATHINFO_EXTENSION);
                              if (in_array(strtolower($frontExt), ['jpg','jpeg','png','webp'])): ?>
                  ...
                  <div>
                      ...
                      <?php if (!empty($member['id_back'])): ?>
                          <div class="mb-2 flex items-center gap-2 bg-slate-50 rounded-lg p-2" id="existing_back_block">
                              <?php 
                              $backExt = pathinfo($member['id_back'], PATHINFO_EXTENSION);
                              if (in_array(strtolower($backExt), ['jpg','jpeg','png','webp'])): ?>
                      ...
                      <div id="id_back_preview" class="mt-2 hidden">
                          <div class="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                              <i class="fa-regular fa-file-image text-green-600"></i>
      ...
      <div class="card">
        ...
        <div class="card-body space-y-5">
          ...
          <div class="p-5 rounded-xl" style="background:#f0fdf9;border:1px solid #a7f3d0;">
            <div class="flex items-center justify-between mb-3"><div><p class="font-semibold text-emerald-800 text-sm">Membership Registration Fee</p><p class="text-xs text-emerald-600">Default: <?= Format::currency((float)($settings['membership_fee'] ?? 10000)) ?></p></div><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="membership_fee_paid" id="feePaid" value="1" <?= !empty($member['membership_fee_paid']) ? 'checked' : '' ?> class="w-5 h-5" style="accent-color:var(--green-mid)"><span class="text-sm font-semibold text-emerald-800">Fee Paid</span></label></div>
...
function validateStep(step) {
  ...
  (required[step] || []).forEach(id => {
    ...
    if (!el || !el.value.trim()) { el && el.classList.add('border-red-400'); valid = false; window.showToast('error', 'Please fill in all required fields.'); }
...
function updateReviewSummary() {
  ...
  const hasExistingAvatar = document.getElementById('photoPreview') && document.getElementById('photoPreview').src;
...
// ID document preview for newly uploaded files
...
// Remove existing file (adds hidden delete field and hides the block)
...
document.getElementById('memberForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      const redirectUrl = data.data?.redirect || '<?= APP_URL ?>/members/<?= $member['id'] ?>';
