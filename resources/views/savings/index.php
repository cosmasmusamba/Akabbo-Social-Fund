...
<div class="flex items-center justify-between mb-5">
  <div>
    <h1 class="text-xl font-bold text-slate-800">Savings Accounts</h1>
    <p class="text-sm text-slate-400 mt-0.5"><?= number_format($result['total']??0) ?> accounts</p>
  </div>
  <div class="flex gap-2">
    ...
    <a href="<?= APP_URL ?>/savings/withdraw" class="btn btn-secondary btn-sm">
      <i class="fa-solid fa-arrow-up-from-bracket"></i> Withdraw
    </a>
    <a href="<?= APP_URL ?>/savings/transfer" class="btn btn-secondary btn-sm">
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
  <?php 
  $stats = [
    ['Total Savings', Format::currencyCompact((float)($summaryStats['total_savings']??0)), 'text-emerald-700', 'fa-piggy-bank'],
    ['Accounts', number_format($summaryStats['total_accounts']??0), 'text-blue-700', 'fa-wallet'],
    ['Active', number_format($summaryStats['active_accounts']??0), 'text-green-600', 'fa-circle-check'],
    ['Dormant', number_format($summaryStats['dormant_accounts']??0), 'text-amber-600', 'fa-moon']
  ];
    ...
    <form method="GET" class="flex flex-wrap gap-3">
      <div class="relative flex-1 min-w-[180px]">
        ...
        <input type="text" name="search" value="<?= htmlspecialchars($_GET['search']??'') ?>" 
               placeholder="Search account, member…" class="form-control pl-9 py-2 text-sm">
      </div>
      <select name="status" class="form-control form-select py-2 text-sm w-auto" onchange="this.form.submit()">
        <option value="">All Status</option>
        <?php foreach(['active','dormant','frozen'] as $s): ?>
      ...
      <tbody>
        ...
        <tr>
          <td colspan="8" class="text-center py-12">
            ...
            <p class="text-slate-400">No savings accounts found</p>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <div class="flex items-center gap-2"><?= Avatar::medium($a) ?><div>
                <div class="text-sm font-semibold"><?= htmlspecialchars($a['member_name']) ?></div>
          ...
          <td>
            <div class="flex gap-1">
              <a href="<?= APP_URL ?>/savings/<?= $a['id'] ?>" class="btn btn-secondary btn-sm py-1 px-2" title="View details">
                <i class="fa-solid fa-eye text-xs"></i>
              </a>
              <a href="<?= APP_URL ?>/savings/statement/<?= $a['id'] ?>" class="btn btn-secondary btn-sm py-1 px-2" target="_blank" title="Statement">
                <i class="fa-solid fa-file-pdf text-xs"></i>
              </a>
              <?php if($auth->can('savings.deposit')): ?>
              <a href="<?= APP_URL ?>/savings/deposit?member=<?= $a['member_id'] ?>&account=<?= $a['id'] ?>" 
                 class="btn btn-sm py-1 px-2" style="background:#dcfce7;color:#166534;border:1px solid #a7f3d0;font-size:0.7rem">
                Dep.
              </a>
              <?php endif; ?>
            </div>
...
async function postInterest() {
  if(!confirm('Post monthly interest to all active savings accounts?\n\nThis will calculate and add interest based on current balances.')) return;
  const fd = new FormData();
  fd.append('csrf_token', document.querySelector('meta[name="csrf-token"]').content);
  try {
    const r = await fetch('<?= APP_URL ?>/savings/post-interest', {
      method: 'POST',
