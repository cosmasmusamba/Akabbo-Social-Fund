<?php use App\Helpers\Format; $pageTitle='Transfer Savings'; $activePage='savings'; $breadcrumbs=['Savings'=>APP_URL.'/savings','Transfer'=>null]; ?>
<div class="max-w-2xl mx-auto">
  <div class="flex items-center justify-between mb-5">
    <div><h1 class="text-xl font-bold text-slate-800">Transfer Savings</h1><p class="text-sm text-slate-400 mt-0.5">Move funds between member accounts</p></div>
  ...
  <div class="card"><div class="card-body">
    <form id="transferForm">
      ...
      <div class="mb-5">
        <label class="form-label required">From Member</label>
      ...
      <div id="fromMemberInfo" class="hidden mb-5 p-4 rounded-xl bg-blue-50 border border-blue-200">
        <div class="flex items-center gap-3"><div class="avatar-circle w-10 h-10" id="fromAvatar"></div><div><div class="font-bold" id="fromName"></div><div class="text-sm text-slate-500" id="fromNo"></div></div></div>
        <div class="mt-2 pt-2 border-t border-blue-200"><div class="text-xs text-slate-500">Available Balance</div><div class="font-bold text-emerald-700 text-lg" id="fromBalance"></div></div>
      </div>
      <div class="mb-5">
        <label class="form-label required">To Member</label>
      ...
      <div id="toMemberInfo" class="hidden mb-5 p-4 rounded-xl bg-emerald-50 border border-emerald-200">
        <div class="flex items-center gap-3"><div class="avatar-circle w-10 h-10" id="toAvatar"></div><div><div class="font-bold" id="toName"></div><div class="text-sm text-slate-500" id="toNo"></div></div></div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div class="col-span-2"><label class="form-label">Description (optional)</label><input type="text" name="description" class="form-control" placeholder="e.g. Internal transfer"></div>
      </div>
      <div class="flex gap-3 justify-end pt-4 border-t border-slate-100">
        <a href="<?= APP_URL ?>/savings" class="btn btn-secondary">Cancel</a>
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-arrow-right-arrow-left"></i> Transfer</button>
      </div>
    </form>
  </div></div>
</div>
...
function searchMember(q, type) {
  if (q.length < 2) { document.getElementById(type+'MemberResults').classList.add('hidden'); return; }
  fetch(`<?= APP_URL ?>/members/search?q=${encodeURIComponent(q)}`, {headers:{'X-Requested-With':'XMLHttpRequest'}})
    .then(r=>r.json()).then(d=>{
      ...
      box.innerHTML=d.data.map(m=>`<div class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 cursor-pointer" onclick='selectMember(${JSON.stringify(m).replace(/"/g,"&quot;")}, "${type}")'><div class="avatar-circle w-8 h-8 text-xs">${m.initials}</div><div><div class="font-semibold text-sm">${m.full_name}</div><div class="text-xs text-slate-400">${m.member_no} · ${m.total_savings_fmt}</div></div></div>`).join('');
...
function selectMember(m, type) {
  ...
  if (type === 'from') document.getElementById('fromBalance').textContent = m.total_savings_fmt;
}
document.addEventListener('click', e=>{ if(!e.target.closest('#fromMemberWrap')) document.getElementById('fromMemberResults')?.classList.add('hidden'); if(!e.target.closest('#toMemberWrap')) document.getElementById('toMemberResults')?.classList.add('hidden'); });

// Transfer form submission with fetch
